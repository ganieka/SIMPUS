  ...
  <body style="font-family:Poppins; margin-top: 75px;"> 
    ...
    if(isset($_SESSION['login_admin'])){ 
        ...
        $sql="SELECT anggota.username,NIM,books.bid,name,authors,approve,issue,issue_book.return,issue_book.returned,issueid,denda,late FROM anggota 
              inner join issue_book ON anggota.username=issue_book.username inner join books ON issue_book.bid=books.bid";
    ...
    <?php   while($row=mysqli_fetch_assoc($res)) 
            {
                if($row['approve']==$exp||$row['approve']==$ret){?> 
                  ...
                  <?php }?>
                      ...
                      <td>
                  <?php if($row['approve'] == $exp || $row['approve'] == $yes) {?>
                          <div class="btn btn-link btn-sm btn-rounded btn-center"> 
                      <?php echo "<a href='confirm.php?issueid=".$row['issueid']."&bid=".$row['bid']."'>Confirm Return</a>";
                        }else{
                          echo "-";
                        }?>
                      </div>
